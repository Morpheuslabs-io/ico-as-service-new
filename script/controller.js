...
function doCheckTokenPairNew(userAddress, token1, token2, fromBlock, toBlock, toTimeStr) {

  return new Promise(function(resolve, reject) {

    if (!isValidAddress(userAddress)) {
      console.log('doCheckTokenPairNew - Invalid address:', userAddress);
      let checkUserResult = `${userAddress},${toTimeStr},Invalid address,`
      return resolve(checkUserResult)
    }

    console.log(`doCheckTokenPairNew - userAddress:${userAddress}, token1:${JSON.stringify(token1)}, token2:${JSON.stringify(token2)}, fromBlock:${fromBlock}, toBlock:${toBlock}`);
    ...
    xhr.get(url)
      .then(function (response) {
        ...
        if (data && data.result && data.result.length > 0) {
            ...
            if (tx.contractAddress === token1.address) {
              ...
              if (tx.from === userAddress) {
                ...
                if (remainingAmount < token1.holdAmount1) {
                  ...
                  resolve(`${userAddress},${toTimeStr},${remark},${tx.hash},,,,No,No,No,No,No,No`)
                }
              }
            }
          ...
          resolve(`${userAddress},${toTimeStr},No transaction found on Etherscan,`)
        }
      })
      .catch(function (error) {
        console.log("doCheckToken error", error);
        resolve(`${userAddress},${toTimeStr},Error: cannot determine due to internal failure,`)
      })
  })
}
